...
public class baseclass_Strings {
			...
			if(obj.getSheetName(i).equalsIgnoreCase("testdata"))  //Check if we are on the desired sheet name
			{
				...
				while(rows.hasNext())
				{
					...
					if(r.getCell(column).getStringCellValue().equalsIgnoreCase(testCaseName))
					{
						...
						while(re.hasNext())
						{
							arr.add(re.next().getStringCellValue());
						}
					}
				}
			}
	...
	{
		baseclass_Strings b = new baseclass_Strings(); //Creating the object of the class which has function for pulling out excel data
		ArrayList data = b.getData("Logout"); // call the method, passing the test case name as the argument, the test case we wish to execute
